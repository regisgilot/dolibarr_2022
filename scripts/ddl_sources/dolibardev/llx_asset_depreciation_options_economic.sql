create table dolibardev.llx_asset_depreciation_options_economic
(
    ...
    accelerated_depreciation_option   int                                   null,
    ...
    constraint uk_asset_doe_fk_asset
        unique (fk_asset),
    constraint uk_asset_doe_fk_asset_model
        unique (fk_asset_model),
    constraint fk_asset_doe_asset
        foreign key (fk_asset) references dolibardev.llx_asset (rowid),
    constraint fk_asset_doe_asset_model
        foreign key (fk_asset_model) references dolibardev.llx_asset_model (rowid),
    constraint fk_asset_doe_user_modif
        foreign key (fk_user_modif) references dolibardev.llx_user (rowid)
);

create index idx_asset_doe_rowid
    on dolibardev.llx_asset_depreciation_options_economic (rowid);
