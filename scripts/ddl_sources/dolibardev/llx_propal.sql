create table dolibardev.llx_propal
(
    rowid                   int auto_increment
        primary key,
    ref                     varchar(30)                               not null,
    entity                  int           default 1                   not null,
    ref_ext                 varchar(255)                              null,
    ref_int                 varchar(255)                              null,
    ref_client              varchar(255)                              null,
    fk_soc                  int                                       null,
    fk_projet               int                                       null,
    tms                     timestamp     default current_timestamp() not null on update current_timestamp(),
    datec                   datetime                                  null,
    datep                   date                                      null,
    fin_validite            datetime                                  null,
    date_valid              datetime                                  null,
    date_signature          datetime                                  null,
    date_cloture            datetime                                  null,
    fk_user_author          int                                       null,
    fk_user_modif           int                                       null,
    fk_user_valid           int                                       null,
    fk_user_signature       int                                       null,
    fk_user_cloture         int                                       null,
    fk_statut               smallint      default 0                   not null,
    price                   double        default 0                   null,
    remise_percent          double        default 0                   null,
    remise_absolue          double        default 0                   null,
    remise                  double        default 0                   null,
    total_ht                double(24, 8) default 0.00000000          null,
    total_tva               double(24, 8) default 0.00000000          null,
    localtax1               double(24, 8) default 0.00000000          null,
    localtax2               double(24, 8) default 0.00000000          null,
    total_ttc               double(24, 8) default 0.00000000          null,
    fk_account              int                                       null,
    fk_currency             varchar(3)                                null,
    fk_cond_reglement       int                                       null,
    deposit_percent         varchar(63)                               null,
    fk_mode_reglement       int                                       null,
    note_private            text                                      null,
    note_public             text                                      null,
    model_pdf               varchar(255)                              null,
    date_livraison          date                                      null,
    fk_shipping_method      int                                       null,
    fk_warehouse            int                                       null,
    fk_availability         int                                       null,
    fk_input_reason         int                                       null,
    fk_incoterms            int                                       null,
    location_incoterms      varchar(255)                              null,
    import_key              varchar(14)                               null,
    extraparams             varchar(255)                              null,
    fk_delivery_address     int                                       null,
    fk_multicurrency        int                                       null,
    multicurrency_code      varchar(3)                                null,
    multicurrency_tx        double(24, 8) default 1.00000000          null,
    multicurrency_total_ht  double(24, 8) default 0.00000000          null,
    multicurrency_total_tva double(24, 8) default 0.00000000          null,
    multicurrency_total_ttc double(24, 8) default 0.00000000          null,
    last_main_doc           varchar(255)                              null,
    online_sign_ip          varchar(48)                               null,
    online_sign_name        varchar(64)                               null,
    constraint uk_propal_ref
        unique (ref, entity),
    constraint fk_propal_fk_projet
        foreign key (fk_projet) references dolibardev.llx_projet (rowid),
    constraint fk_propal_fk_soc
        foreign key (fk_soc) references dolibardev.llx_societe (rowid),
    constraint fk_propal_fk_user_author
        foreign key (fk_user_author) references dolibardev.llx_user (rowid),
    constraint fk_propal_fk_user_cloture
        foreign key (fk_user_cloture) references dolibardev.llx_user (rowid),
    constraint fk_propal_fk_user_signature
        foreign key (fk_user_signature) references dolibardev.llx_user (rowid),
    constraint fk_propal_fk_user_valid
        foreign key (fk_user_valid) references dolibardev.llx_user (rowid)
)
    charset = latin1;

create index idx_propal_fk_account
    on dolibardev.llx_propal (fk_account);

create index idx_propal_fk_currency
    on dolibardev.llx_propal (fk_currency);

create index idx_propal_fk_projet
    on dolibardev.llx_propal (fk_projet);

create index idx_propal_fk_soc
    on dolibardev.llx_propal (fk_soc);

create index idx_propal_fk_user_author
    on dolibardev.llx_propal (fk_user_author);

create index idx_propal_fk_user_cloture
    on dolibardev.llx_propal (fk_user_cloture);

create index idx_propal_fk_user_valid
    on dolibardev.llx_propal (fk_user_valid);

create index idx_propal_fk_warehouse
    on dolibardev.llx_propal (fk_warehouse);

